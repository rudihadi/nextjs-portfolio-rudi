...
const HeaderLink: React.FC<{ item: HeaderItem }> = ({ item }) => {
  ...
  return (
    ...
    >
      <Link
        href={item.href}
        className={`text-base flex py-2 font-normal hover:text-primary dark:hover:text-primary text-black dark:text-white  ${path === item.href ? "text-primary dark:text-primary!" : "  "} ${path.startsWith("/about") && item.href === "/about" ? "text-primary! dark:text-primary!" : null} ${path.startsWith("/portfolio") && item.href === "/portfolio" ? "text-primary! dark:text-primary!" : null} ${path.startsWith("/contact") && item.href === "/contact" ? "text-primary! dark:text-primary!" : null}`}
      >
        {item.label}
        {item.submenu && (
          <svg
